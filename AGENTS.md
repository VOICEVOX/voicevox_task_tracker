...
- 新規ソースファイルは400行を超えた時点で責務分割を検討する。1000行を超える新規ソースファイルは作らない
- 既存の巨大ファイルへ新しい責務を追加しない
- 個人催促の最終結果は`src/cli/personal-reminder`へ抽出し、`src/cli/personal-reminder-runtime.ts`と`src/cli/production-runtime.ts`の行数を純減させる
- リポジトリ全体の既存1000行超過は、関心事ごとに別作業で解消する
