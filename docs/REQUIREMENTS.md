...
| `COL-019` | MUST | 増分収集 — 前回成功時刻とitem fingerprintを使い、変更項目、依存隣接項目、AI分析の再試行項目だけを詳細再取得できなければならない。                      | `AT-COL-019`: 1000項目中10変更fixtureで詳細取得が変更10件、依存隣接、AI分析の再試行項目だけに限定される。 |
...
| `AIC-017` | MUST | 予算超過優先順位 — 予算不足時は曖昧な状態判定候補、owner unknown、changed blockers、downstream impact順に分析し、残りをdeferred表示しなければならない。                                                                                                                                                                                                                                         | `AT-AIC-017`: 10候補/3call上限fixtureで上位3件が選ばれる。                                                                                                                                            |
...
| `AIC-023` | MUST | 失敗と延期の再試行 — 前回の`aiAnalysis.status`が`failed`か`deferred`の項目は、GitHub側に変化がなくても次回runの増分詳細取得とAI分析候補へ必ず含めなければならない。terminal項目も除外してはならない。                                                                                                                                                                                           | `AT-AIC-023`: `failed`と`deferred`の未変更terminal fixtureが増分窓で詳細取得され、十分な予算の次回runでAI分析を実行して`used`になる。                                                                 |
...
| `WEB-002` | MUST | 概要ページの絞り込み — 概要ページは対応が必要な項目の一覧を主とし、データの観測時刻と鮮度およびAI状態の注意だけを添えなければならない。各項目には要対応度、重要度、主な待ち相手とその理由、停滞時間を示し、集計値や候補の全列挙を並べてはならない。                                                                                                                                                                                                                                                                                      | `AT-WEB-002`: 概要ページに要対応項目、観測時刻、必要な注意だけが現れ、要対応度と重要度が表示され、待ち相手は主候補1件と残り件数で示される。                                                                                                                                                |
| `WEB-003` | MUST | 要対応一覧 — 要対応度levelがhighまたはmediumで、観測値がfreshの非terminal項目だけを表示しなければならない。並び替えキーは要対応度、重要度、停滞時間の三つだけとし、既定は要対応度の降順にしなければならない。                                                                                                                                                                                                                                                                                                                            | `AT-WEB-003`: level境界、stale、terminalを含むfixtureで表示対象が一致し、三つのキーと方向をURLから復元でき、既定は要対応度scoreの降順になる。                                                                                                                                              |
...
| `WEB-008` | MUST | 一覧の絞り込みと並び替え — repo、type、status、importance、waitingOn、stall、AI利用状況で絞り込める項目一覧を提供しなければならない。並び替えキーは要対応度、重要度、停滞時間の三つだけとし、既定は要対応度の降順にしなければならない。AI分析に失敗または延期し、AI推定が最新でない項目には警告アイコンを表示しなければならない。確定ルールだけで判定してAI推定を意図的に省いた項目と区別し、どちらも絞り込めなければならない。値が有限の列は公開データに実在する選択肢から選ばせ、URLへは表示文言ではなく識別子を入れなければならない。 | `AT-WEB-008`: keyboardのみで全filterとitem遷移ができ、`ai=outdated`で`failed`と`deferred`だけ、`ai=skipped`で`not_required`だけが表示される。警告アイコンは前者だけに現れる。要対応度と重要度はscoreの数値順でsortされ、三つの並び替えだけをURLから復元し、選択肢に無いURL値は捨てられる。 |
| `WEB-009` | MUST | item詳細 — 各item詳細にGitHub URL、状態、要対応度のscoreとlevel、重要度のscore、level、内訳、waitingOn、next action、停滞時間、blocker、evidence、判定の確度区分、履歴を表示しなければならない。AI分析に失敗または延期した項目には、最新のAI推定を反映できなかった旨を警告として1行で表示しなければならない。確定ルールだけで判定してAI推定を省いた項目には、その旨を警告ではない情報として1行で表示しなければならない。GitHubを開けば分かる値や、他の欄が具体的に示している値を重ねて並べてはならない。                                 | `AT-WEB-009`: 任意itemで要対応度と、重要度の各要因と加点を含む必須欄が確認でき、`failed`と`deferred`では警告の注記が、`not_required`では警告ではない注記が現れ、ラベルやassigneeの再掲が現れない。                                                                                         |
...
| `WEB-012` | MUST | 鮮度表示 — データ全体のobservedAtを相対時間で示してJST絶対時刻を補助へ添え、取得できなかったリポジトリがあるときは対象名を添えた注意を示さなければならない。AI分析を設定で無効にしたときだけ概要へ全体の注意を示し、AI推定が最新でない項目は一覧の各行へ警告アイコンで示さなければならない。                                                                                                                                                                                                                                             | `AT-WEB-012`: stale repositoryとAI無効のfixtureが最新やAI有効と誤認できない表示になり、注意へリポジトリ名が現れる。`failed`と`deferred`の項目では各行に警告アイコンが現れ、AIの利用不可と縮退だけでは概要へ全体の注意が現れない。                                                          |
...
| `WEB-014` | MUST | 担当者別の停滞一覧 — 待ち相手を個人とteamへ解決した担当者一覧と、個人ごとの停滞項目一覧を提供しなければならない。個人ごとのページは閲覧者が選んだ所属teamへの待ちも合流させ、その選択をURLで共有できなければならない。個人ごとのページは要対応度、重要度、停滞時間の三つだけで並び替えでき、既定を要対応度の降順とし、並び順をURLで共有できなければならない。                                                                                                                                                                            | `AT-WEB-014`: 待ち相手fixtureで担当者一覧の件数と個人ごとのページの項目数が一致し、team選択と三つの並び順を含むURLを開き直しても同じ項目が同じ順序で出る。                                                                                                                                 |
...
| `WEB-016` | MUST | 項目一覧の一貫性 — 複数の項目一覧では、同じ概念を同じ語彙、同じ視覚表現、同じ並び順で示さなければならない。表示形式は画面幅で決め、ページで決めてはならない。ページの目的から説明できる違いだけを許し、それ以外の差を設けてはならない。                                                                                                                                                                                                                                                                                                  | `AT-WEB-016`: 概要、全項目一覧、担当者個別を複数の画面幅で表示し、同じ概念の語彙、視覚表現、並び順が一致する。表示形式は同じ画面幅で一致し、各ページの差は目的に必要な要素だけになる。                                                                                                     |
