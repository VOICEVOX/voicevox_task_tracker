...
| 停滞レベル          | Discord通知に使う停滞の段階。画面では対象外、注意、緊急、深刻の順に上がる。内部名は`severity` |
...
| `DAT-004` | MUST | 日次履歴 — 日次差分またはevent historyを日付単位で保持し、previous→currentを再構成できなければならない。Discordへ送信した項目通知は送信時点の表示情報と理由をeventとして追記しなければならない。schema version 1と2からの移行では通知eventを追加してはならない。 | `AT-DAT-004`: 任意2日間のowner/edge/severity差分を再生できる。再通知は別eventとして残り、旧schemaの履歴を移行した場合は通知eventが0件になる。            |
