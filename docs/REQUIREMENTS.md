...
| `DAT-003` | MUST | current snapshot — tracker-stateにschema version 7のcurrent snapshotをcanonical JSONで保存し、各追跡項目に`aiAnalysis.status`を持たせなければならない。                                                | `AT-DAT-003`: 同一入力2回でvolatile fieldを除くbyte列が一致し、snapshot schemaが6種類のAI利用状況だけを受理する。 |
