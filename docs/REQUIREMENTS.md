...
| `WEB-008` | MUST | 一覧の絞り込み — repo、type、status、importance、waitingOn、stallでsort/filterできる項目一覧を提供しなければならない。値が有限の列は公開データに実在する選択肢から選ばせ、URLへは表示文言ではなく識別子を入れなければならない。既定の並び順は停滞時間の降順とする。                                                                            | `AT-WEB-008`: keyboardのみで全列filterとitem遷移ができ、importanceはscoreの数値順でsortされ、選択肢に無いURL値は不正値として捨てられる。             |
| `WEB-009` | MUST | item詳細 — 各item詳細にGitHub URL、状態、重要度のscore、level、内訳、waitingOn、next action、停滞時間、blocker、evidence、判定の確度区分、履歴を表示しなければならない。GitHubを開けば分かる値や、他の欄が具体的に示している値を重ねて並べてはならない。                                                                                       | `AT-WEB-009`: 任意itemで重要度の各要因と加点を含む必須欄が確認でき、ラベルやassigneeの再掲が現れない。                                               |
...
| `WEB-012` | MUST | 鮮度表示 — データ全体のobservedAtを相対時間で示してJST絶対時刻を補助へ添え、取得できなかったリポジトリがあるときは対象名を添えた注意を示し、AIの無効、利用不可、縮退を区別して表示しなければならない。                                                                                                                                         | `AT-WEB-012`: stale repoと3種類のAI状態のfixtureが最新や完全成功と誤認できない表示になり、注意へリポジトリ名が現れる。                               |
