...
| `NTF-010` | MUST | 重複抑制 — notification ledgerの予約は24時間だけ再送を抑え、期限切れ後は未送信候補を再試行可能にしなければならない。送信済みまたは手動抑制済みの同じnotification keyは期限なく抑制し、状態変化で別keyになった候補は送信対象に戻さなければならない。 | `AT-NTF-010`: 期限内と期限切れの予約、送信済みと手動抑制済みのkey、状態変化後の別keyで期待回数になる。 |
...
| `DAT-004` | MUST | 日次履歴 — 日次差分またはevent historyを日付単位で保持し、previous→currentを再構成できなければならない。Discordへ送信した項目通知は送信時点の表示情報と理由をeventとして追記しなければならない。schema version 1と2からの移行では通知eventを追加してはならない。 | `AT-DAT-004`: 任意2日間のowner/edge/severity差分を再生できる。同じ項目への別の通知は別eventとして残り、旧schemaの履歴を移行した場合は通知eventが0件になる。 |
| `DAT-005` | MUST | AI cacheと通知ledger — AI cache、analysis metadata、予約期限と送信結果を持つnotification ledgerをstate branchで保持しなければならない。                                                                                                                          | `AT-DAT-005`: runnerを破棄して再実行してもcache hit、予約期限、送信済みと手動抑制済みのkeyが維持される。                                                    |
