...
| 停滞レベル          | Discord通知に使う停滞の深刻さ。内部データでは`severity`と表す。none、watch、urgent、criticalの4段階 |
| 通知管理記録        | Discord通知の予約、送信結果、再通知抑制期限、手動抑制を保持する記録                                 |
...
statusは待たれている行動を表す。誰を待っているかは待ち相手を表す`waitingOn`だけで表し、status名へ主体を含めない。
...
blocked parentは「親自身を毎日催促」せず、blockerの停滞レベルとdownstream impactを通知順位へ使う。priority labelは停滞レベルを最大1段階引き上げられるが、低信頼AIだけでcriticalへ引き上げない。
...
通知0件なら投稿しない。同じ通知理由と状態を表すnotification keyは一度だけ送信する。
...
| `COL-019` | MUST | 増分収集 — 前回成功時点のitem fingerprintを使い、変更項目、依存隣接項目、AI分析の再試行項目だけを詳細再取得できなければならない。                      | `AT-COL-019`: 1000項目中10変更fixtureで詳細取得が変更10件、依存隣接、AI分析の再試行項目だけに限定される。 |
...
| `RSP-024` | MUST | 責務遷移時刻 — 待ち相手を表す`waitingOn`の実体またはstatusが変わった時点でownerSince/stallSinceを更新しなければならない。                                                                                                                                 | `AT-RSP-024`: 担当決め待ちのメンテナ候補からrequested reviewerへ責務が移るfixtureで時刻がreview requestになる。                               |
...
| `AIC-023` | MUST | 失敗と延期の再試行 — 前回の`aiAnalysis.status`が`failed`か`deferred`の項目は、GitHub側に変化がなくても次回runの全履歴詳細取得とAI分析候補へ必ず含めなければならない。terminal項目も除外してはならない。                                                                                                                                                                                         | `AT-AIC-023`: `failed`と`deferred`の未変更terminal fixtureが全履歴で詳細取得され、十分な予算の次回runでAI分析を実行して`used`になる。                                                                 |
...
| `WEB-016` | MUST | 項目一覧の一貫性 — 項目一覧の表とカードは、項目、待ち相手と状態、要対応度、重要度、期限、停滞時間の順にしなければならない。複数の項目一覧では、同じ概念を同じ語彙、同じ視覚表現、同じ並び順で示さなければならない。表示形式は画面幅で決め、ページで決めてはならない。表は固定レイアウトとし、項目を最も広く、待ち相手と状態を次に広く、四つの指標列を狭くしなければならない。数値は等幅数字で中央または右へ揃えなければならない。ページの目的から説明できる違いだけを許し、それ以外の差を設けてはならない。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | `AT-WEB-016`: 項目一覧と担当者個別を複数の画面幅で表示し、同じ六つの情報が同じ語彙、視覚表現、並び順で現れる。広い画面では項目の題名へ最も広い幅が割り当てられ、狭い画面では同じ順のカードになる。各ページの差は主候補の選び方など目的に必要な要素だけになる。                                                                                                                                                                                                                                                                 |
| `WEB-017` | MUST | 通知履歴 — `/notification-history`を別ページとして提供し、Discordへ送信済みの項目通知を新しい順に表示しなければならない。通知日時、項目の参照とタイトル、GitHub URL、通知理由を表示し、送信履歴がなければ空状態を表示しなければならない。過去schemaの通知を推測して表示してはならない。通知はPages公開後に確定するため、送信後の次回runから表示してよい。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `AT-WEB-017`: 通知履歴のdeep linkが有効になり、送信済み通知が日時の降順で現れる。項目のGitHub URLと理由を確認でき、過去schemaまたは通知0件のfixtureでは空状態が現れる。                                                                                                                                                                                                                                                                                                                                                        |

設定したメンテナのGitHubユーザー名は待ち相手を表す`waitingOn`の`kind=user`として公開し、他のuser候補と同じ担当者ページ、リンク、アバターを使う。
担当者一覧は公開summaryの待ち相手を表す`waitingOn`にあるuserとteamを別の待ち相手として集計し、teamをmemberへ展開しない。
...
| `NTF-007` | MUST | digest構成 — digestを「停止要因」「内容確認または担当が未確定」「新規解消・重要変化」に分け、各itemにrepo#number、title、待ち相手を表す`waitingOn`、duration、reason、URLを含めなければならない。                                            | `AT-NTF-007`: payload snapshotが必須項目を満たす。                                                     |
...
| `NTF-010` | MUST | 重複抑制 — 通知管理記録の予約は24時間だけ再送を抑え、期限切れ後は未送信候補を再試行可能にしなければならない。送信済みまたは手動抑制済みの同じnotification keyは期限なく抑制し、状態変化で別keyになった候補は送信対象に戻さなければならない。 | `AT-NTF-010`: 期限内と期限切れの予約、送信済みと手動抑制済みのkey、状態変化後の別keyで期待回数になる。 |
...
| `DAT-003` | MUST | current snapshot — tracker-stateにschema version 10のcurrent snapshotをcanonical JSONで保存し、各追跡項目に`attention`、期限日を持つ`deadlineAssessment`、`aiAnalysis.status`を持たせなければならない。                                                          | `AT-DAT-003`: 同一入力2回でvolatile fieldを除くbyte列が一致し、snapshot schemaが期限日、`attention`のscoreとlevel、および6種類のAI利用statusを検証する。  |
...
| `DAT-005` | MUST | AI cacheと通知管理記録 — AI cache、analysis metadata、予約期限と送信結果を持つ通知管理記録をstate branchで保持しなければならない。                                                                                                                               | `AT-DAT-005`: runnerを破棄して再実行してもcache hit、予約期限、送信済みと手動抑制済みのkeyが維持される。                                                  |
