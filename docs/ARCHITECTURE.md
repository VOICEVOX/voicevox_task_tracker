...
12. PagesとDiscordの完了後に、新しいGitHub収集cacheとAI cacheだけをcanonical JSONで`tracker-state`へ保存します。run reportはActions artifactとjob summaryへ保存します。
...
`tracker-state` branchの配置は次の4種類だけです。
