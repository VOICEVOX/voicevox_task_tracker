...
runtimeの分析対象に含まれる項目は、今回の代表候補に現れない未終了の旧原因も最新入力で再評価します。たとえば同じPRがreview待ちに変わった場合も、継続中のmerge原因をそのreview待ちとして評価できます。それ以外の追跡項目は前回snapshotの原因・根拠・採用済み評価を保持し、原因の入力が不完全な項目は状態がterminalでも`personalReminderCausePlanning`を`pending`にします。取得だけでは評価完了にせず、保持する採用値を`unknown`へ書き換えません。原因がないterminal項目だけを`excluded`へ更新して列挙対象から除外します。
...
現行の汎用AI出力構造は、domainの要素別resultスキーマを正本とします。選択要素用のJSON Schemaと受信側Zodスキーマは、この正本と共通ルートフィールドから組み立てます。送信時のresultのフィールド順は根拠などを先、valueを最後とし、正本の定義順には依存させません。選択要素用のJSON Schemaでは選択した要素だけを必須にします。移行済みの保存値は別の受理条件を持つため、この新規出力スキーマへ統一しません。生成と検証で正本を共有しても、根拠の実在・一意性・採用条件を扱う意味検証は省略しません。
