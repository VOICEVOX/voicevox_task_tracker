...
意味上の判定規則を変えるときは`ai.promptVersion`を上げ、`ai.promptUpdates`へ変更前の`fromVersion`、変更後の`toVersion`、変更が影響する項目を表す`scope`を追加します。
複数の更新をまたぐ結果も扱えるよう、更新履歴は連続させ、末尾の`toVersion`を`ai.promptVersion`に合わせてください。
記録済みの更新内容を書き換えず、新しいversionへの更新を追加します。

| scope                   | 使用できる変更                                                                         |
| ----------------------- | -------------------------------------------------------------------------------------- |
| `all`                   | 共通規則の変更や、影響する項目を限定できない変更                                       |
| `existing_deadline`     | 保存済みの期限日の誤採用を除く変更。期限がない結果から新たな期限を拾う変更には使わない |
| `open_unassigned_issue` | openかつ未アサインのIssueだけに影響する変更                                            |

`existing_deadline`は、保存済みの期限日がある項目だけを対象にします。
期限日の判定結果がない項目や、期限日が`null`の項目は対象外です。
週や月の期間表現を新たに期限へ変換する変更には`all`を使ってください。
`open_unassigned_issue`は、変更前後のどちらかで条件に当てはまる項目を対象にします。
close済みやmerge済みでも、変更が影響する判定を持つ項目は再推論の対象です。

履歴を空配列にした場合や、保存済みのversionから現在までの更新を確認できない場合は、以前のプロンプトによる結果を再利用しません。
変更対象外と判断できても、本文、コメント、AI入力、隣接graph、model、reasoning effort、backend、schemaなどが変われば通常の再推論が必要です。
特定の項目にしか使わない入力フィールドを全項目へ追加すると、入力hashの変更によって対象外の項目も再推論されます。対象の限定は、用途に合う入力設計と併せて行ってください。

変更対象外で入力も一致する項目は、検証を通した元のcacheを使います。
生成時のversion、実行時刻、cache keyを保持し、新しいプロンプトで生成した結果として記録しません。
新規分析、失敗・延期の再試行、元のcacheを利用できない場合の分析には、現在のプロンプトを使います。
選ばれた項目は1回の呼び出しで全出力を判定するため、同じ項目の期限以外の判定も更新されます。
