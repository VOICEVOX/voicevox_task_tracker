...
概要 `/`、全項目一覧 `/items`、担当者個別 `/people/{login}` の項目一覧では、次の規約を守ります。

- `ResponsiveTableCardList`は画面幅だけで表とカードを切り替え、ページの種類では切り替えません。
- 切り替え幅は`breakpoint`で呼び出し側が明示します。
- 表の列は「項目、状態、要対応度、重要度、待ち相手、停滞時間」の順に置き、ページに不要な列だけを省きます。
...
- バッジ内のラベルは、列見出しがある表では省き、列見出しがないカードでは表示します。
- 点数は項目詳細だけに表示し、一覧には表示しません。
- 要対応度のバッジは塗り、重要度のバッジは枠線で表し、色を見分けられなくても二つの指標を区別できるようにします。
- 重要度が低の場合もバッジを表示し、未算出と区別します。
- GitHubへの導線は題名の隣に置くアイコンボタン一つだけにし、44px以上の操作領域を確保します。
- 件数と並び替えは検索や絞り込みなどのページ固有の操作の後、一覧の直前に置きます。
- 観測時刻は公開データ全体の属性として、共通ヘッダーの一か所だけに表示します。
- 表の列見出しは画面上端に固定します。
- 待ち相手を指す表示は「待ち相手」、要対応度を指す表示は「要対応度」に統一します。
